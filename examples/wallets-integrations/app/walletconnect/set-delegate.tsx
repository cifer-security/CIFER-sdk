 ...
 *
 * For WalletConnect, we use the same EIP-1193 method as MetaMask:
 *   provider.request({ method: 'eth_sendTransaction', params: [tx] })
 *
 * This works because the WalletConnect EthereumProvider implements the
 * same EIP-1193 interface as MetaMask — the only difference is how the
 * provider was created (via QR code pairing vs. browser extension).
 *
 * SDK function used:
...
export function SetDelegate({ sdk, chainId, address, provider, log }: SetDelegateProps) {
  // ---- Local state ----
  ...
  //
  // Step 2: Send the TxIntent via the WalletConnect provider.
  //   This is identical to MetaMask — both are EIP-1193 providers!
  //   The user will approve the transaction in their mobile wallet.
  // =========================================================================
  ...
  }, [sdk, chainId, address, provider, secretId, newDelegate, log])
  ...
  return (
    <div className="glow-card p-6">
      ...
      <p className="text-sm text-zinc-400 mb-4">
        Build a transaction intent with the SDK, then send it via
        WalletConnect. Only the secret owner can set a delegate.
      </p>
      ...
      <div className="text-xs font-mono text-zinc-600 bg-zinc-900/50 rounded p-3 mb-4">
        ...
        <br />
        <br />
        {`// 2. Send via WalletConnect (same EIP-1193 as MetaMask!)`}
        <br />
        {`const hash = await provider.request({`}
        <br />
        {`  method: 'eth_sendTransaction',`}
