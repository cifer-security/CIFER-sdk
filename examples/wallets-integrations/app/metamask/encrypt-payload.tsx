...
export function EncryptPayload({ sdk, chainId, address, log }: EncryptPayloadProps) {
  // ---- Local state ----
  ...
  //
  // blackbox.payload.encryptPayload() sends the plaintext to the Blackbox
  // which encrypts it using the secret's ML-KEM public key.
  //
  // Authentication: the Blackbox requires a signed "data string" proving
  // the caller is the owner or delegate. The SDK handles this internally
  // using the signer we provide.
  //
  // The signer is created from MetaMask's window.ethereum provider using
  // the Eip1193SignerAdapter from the SDK.
  // =========================================================================
  ...
  }, [sdk, chainId, secretId, plaintext, log])
