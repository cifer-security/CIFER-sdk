/**
 * Fetch Secret Creation Fee — MetaMask
 * ======================================
 *
...
export function FetchFee({ sdk, chainId, log }: FetchFeeProps) {
  ...
  // (Parent passes a new chainId — we clear stale data)
  ...
  const fetchFee = useCallback(async () => {
    try {
      ...
      // Get the controller address for this chain (discovered during SDK init)
      const controllerAddress = sdk.getControllerAddress(chainId)
      log(`Controller address: ${controllerAddress}`)

      // Call the SDK's getSecretCreationFee — a read-only on-chain call
  ...
  // =========================================================================
  // UI — renders a card with the fee result or a Fetch button
  // =========================================================================
  return (
