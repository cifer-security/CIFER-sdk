/**
 * Encrypt File — MetaMask
 * ========================
 *
 * This component demonstrates flows.encryptFileJobFlow() —
 * encrypting a file using a CIFER secret via the Blackbox API.
 *
 * The flow has 3 steps:
 *   1. Upload — sends the file to the Blackbox for encryption
 *   2. Poll   — waits for the encryption job to complete
 *   3. Download — fetches the encrypted result
 *
 * For MetaMask, we create the signer with:
 *   new Eip1193SignerAdapter(window.ethereum)
 *
 * SDK function used:
 *   flows.encryptFileJobFlow(ctx, { secretId, file }, options)
 *
 * Returns:
 *   { jobId, job, encryptedFile: Blob }
 */

"use client"

import { useState, useCallback, useRef } from "react"
import { CheckCircle, Loader2, Lock, Upload, FileUp, Download, X } from "lucide-react"
...
export function EncryptFile({ sdk, chainId, address, log }: EncryptFileProps) {
  // ---- Local state ----
  ...
  // =========================================================================
  // Encrypt file via the SDK flow
  //
  // flows.encryptFileJobFlow() orchestrates:
  //   1. Upload the file for encryption (authenticated via signer)
  //   2. Poll the job status until complete
  //   3. Download the encrypted result
  //
  // The signer is created from MetaMask's window.ethereum provider.
  // =========================================================================
  const handleEncrypt = useCallback(async () => {
    ...
    try {
      ...
      setSteps([
        ...
        { id: "download", description: "Download encrypted file", status: "pending" },
      ])

      log(`Encrypting file "${file.name}" (${(file.size / 1024).toFixed(1)} KB) with secret #${secretId}...`)
      ...
      const ctx: flows.FlowContext = {
        ...
        logger: (msg) => {
          log(msg)
          // Parse "Progress: N%" for the progress bar
      ...
      const result = await flows.encryptFileJobFlow(
        ctx,
      ...
      if (!result.success || !result.data) {
        throw result.error ?? new Error("Encryption flow failed")
      }

      // Auto-download the encrypted file
      const outputName = `${file.name}.cifer.zip`
      const url = URL.createObjectURL(result.data.encryptedFile)
      ...
      setDone(true)
      log(`Encrypted file downloaded as "${outputName}"`)
    } catch (err) {
  ...
  return (
    <div className="glow-card p-6">
      ...
      <p className="text-sm text-zinc-400 mb-4">
        Encrypt a file using a secret&apos;s key via the Blackbox API.
        The encrypted file will be downloaded as <code className="text-zinc-300 font-mono text-xs">.cifer.zip</code>.
      </p>
      ...
      <div className="text-xs font-mono text-zinc-600 bg-zinc-900/50 rounded p-3 mb-4">
        ...
        {`);`}
        <br />
        {`// result.data.encryptedFile → Blob`}
      </div>
          ...
          {file ? (
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">
                Drop a file here or <span className="text-[#00ff9d]">browse</span>
              </p>
              <p className="text-xs text-zinc-600 mt-1">Any file type</p>
            </div>
          )}
        </div>
      </div>
